...
@section('content')
	@foreach ($data as $postTerbaru)
	<div class="col-md-6">
		<div class="post">
			<a class="post-img" href="{{ route('blog.isi', $postTerbaru->slug) }}"><img src="{{ $postTerbaru->gambar }}" alt=""></a>
			<div class="post-body">
				<div class="post-category">
					<a href="category.html">{{ $postTerbaru->category->name }}</a>
				</div>
				<h3 class="post-title"><a href="#">{{$postTerbaru->judul}}</a>
				</h3>
				<ul class="post-meta">
					<li><a href="author.html">{{ $postTerbaru->users->name }}</a></li>
					<li>{{ $postTerbaru->created_at->diffForHumans() }}</li>
				</ul>
			</div>
		</div>
	</div>
